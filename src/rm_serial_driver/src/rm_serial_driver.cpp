...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try {
    ...
    // std::cout<<"receive_pitch_   "<<receive_pitch_<<std::endl;
    // std::cout<<"packet.pitch_angle - 90   "<<packet.pitch_angle - 90<<std::endl;
...
void RMSerialDriver::sendDataOfRune(auto_aim_interfaces::msg::TargetRune::SharedPtr msg)
{
  try {
    ...
    if (msg->pitch > 1000 && msg->yaw > 1000) {
      ...
      packet.yaw_angle = msg->yaw + rune_yaw_fix_ ;           //
      packet.pitch_angle = msg->pitch + 90 + rune_pitch_fix_;  //
...
void RMSerialDriver::getParams()
{
  ...
  try {
    rune_yaw_fix_ = declare_parameter<double>("rune_yaw_fix", 0);
  } catch (rclcpp::ParameterTypeException & ex) {
    RCLCPP_ERROR(get_logger(), "The yaw_fix_ provided was invalid");
    throw ex;
  }
  ...
  try {
    rune_pitch_fix_ = declare_parameter<double>("rune_pitch_fix", 0);
  } catch (rclcpp::ParameterTypeException & ex) {
    RCLCPP_ERROR(get_logger(), "The pitch_fix_ provided was invalid");
    throw ex;
  }
