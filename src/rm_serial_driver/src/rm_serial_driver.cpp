  ...
  serial_driver_{new drivers::serial_driver::SerialDriver(*owned_ctx_)}
{
  ...
  target_sub_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
    "/tracker/target", rclcpp::SensorDataQoS(),//从tracker订阅最终锁定目标的信息
...
void RMSerialDriver::receiveData()
{
  ...
  while (rclcpp::ok()) {
    try {
      ...
      //first_time = final_time;
        if(data[9]!=0xAF)
        {
          // std::cout<<"qqqqqqqqqqqqqq"<<std::endl<<std::endl<<std::endl<<std::endl<<std::endl;
          qqqqqqqqqqqqqqqq = qqqqqqqqqqqqqqqq +1;
          continue;
        }
        // printf("1   %d\n",data[0]);
        // printf("2   %d\n",data[1]);
        ...
        // printf("t.transform.rotation.x %f\n", t.transform.rotation.x);
        // printf("t.transform.rotation.y %f\n", t.transform.rotation.y);
        // printf("t.transform.rotation.z %f\n", t.transform.rotation.z);
        // printf("t.transform.rotation.w %f\n", t.transform.rotation.w);
  ...
  if(rclcpp::ok()!=true){
    // std::cout<<"qqqqqqqqqqqqqqqq :  "<< qqqqqqqqqqqqqqqq <<std::endl;
  } 
}
...
void RMSerialDriver::sendData(const auto_aim_interfaces::msg::Target::SharedPtr msg)
{
  ...
  try {
    ...
    geometry_msgs::msg::Point p_w;//position_world
    ...
    // std::cout<<"center_yaw        :"<<center_yaw / M_PI * 180<<std::endl;
    ...
    // pre_armor_yaw[idx] = pre_armor_yaw[idx]>0 ? pre_armor_yaw[idx] : pre_armor_yaw[idx] * (-1);
    pre_armor_yaw[idx] = pre_armor_yaw[idx] * 180/ M_PI;
        // std::cout<<"pre_armor_yaw[idx]        :"<<pre_armor_yaw[idx]<<std::endl;
    ...
    double a_distance = sqrt(p_w.x * p_w.x + p_w.y * p_w.y );
    ...
    // std::cout<<"armor_yaw_err   "<<armor_yaw_err<<std::endl;
    // std::cout<<"armor_pitch_error   "<<armor_pitch_error<<std::endl;
    ...
    // std::cout<<"detect_flag:    "<<packet.detect_flag<<"    shoot_flag:    "<<packet.shoot_flag<<std::endl;
    // std::cout<<"shoot_flag:    "<<packet.shoot_flag<<std::endl;
...
void RMSerialDriver::getParams()
{
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The yaw_fix_ provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The pitch_fix_ provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The large_length provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The large_z provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The small_length provided was invalid");
    throw ex;
  }
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "The small_z provided was invalid");
    throw ex;
  }
...
double RMSerialDriver::dynamicCalcPitchOffset(Eigen::Vector3d & xyz)
{
  ...
  // auto dist_horizonal = sqrt(xyz.squaredNorm() - dist_vertical * dist_vertical);
  // auto dist_horizonal = 25.0;
  // auto bullet_speed = 16.0;
  ...
  // std::cout<<"pitch_new:::::::::::::"<<pitch_new - pitch<<std::endl;
