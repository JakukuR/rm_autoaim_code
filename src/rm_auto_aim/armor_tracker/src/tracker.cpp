...
void Tracker::init(const Armors::SharedPtr & armors_msg)
{
  ...
  tracked_type = tracked_armor.type;
...
void Tracker::update(const Armors::SharedPtr & armors_msg)
{
  ...
  if (!armors_msg->armors.empty()) {
    ...
    if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_) {
      ...
      double measured_yaw = getArmorYaw(tracked_armor);
...
void Tracker::handleArmorJump(const Armor & current_armor)
{
  ...
  double yaw = getArmorYaw(current_armor);
  target_state(6) = yaw;
...
/**
 * @brief Convert orientation to yaw
 * 
 * @param q 
 * @return double 
 */
...
double Tracker::getArmorYaw(const Armor & fit)
{
  fit_yaw = fit.fit_yaw;
  fit_yaw = last_yaw_ + angles::shortest_angular_distance(last_yaw_, fit_yaw);
  last_yaw_ = fit_yaw;
  return fit_yaw;
}
