...
bool rm_auto_aim::RuneDetector::imageProcess(cv::Mat & src)
{
  ...
  if (rune_color == self_BLUE)
    rm_auto_aim::RuneDetector::binary_img = channels.at(2) - channels.at(0);
  else
    binary_img = channels.at(0) - channels.at(2);
...
std::vector<rm_auto_aim::ShootFan> rm_auto_aim::RuneDetector::fillContour()
{
  ...
  // cv::imshow("11111",filled_contour_img);
  ...
  // cv::imshow("1111",filled_contour_img);
    ...
    for (const auto & contour : flow_roi_contours) {
      ...
      if (
        ...
        if (
          ...
          for (int i = 0; i < 4; i++) {
            line(
              rune_debug, sorted_pts[i], sorted_pts[(i + 1) % 4],
              cv::Scalar(0, 255, 0) , 2);  //四个角点连成线，最终形成旋转的矩形。
          }
          // cv::imshow("rune_debug",rune_debug);
