...
: Node("armor_detector", options)
{
  ...
  // std::string videoName = "videoSrc_"+std::string(sstime);

  // vw.open("../VideoSaver/" + videoName + ".avi",cv::VideoWriter::fourcc('M','J','P','G'),60,cv::Size(1440,1080)); 
  ...
  // save_video_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
  //   "/image_raw", rclcpp::SensorDataQoS(),
  //   std::bind(&ArmorDetectorNode::savevideoCallback, this, std::placeholders::_1));
...
// void ArmorDetectorNode::savevideoCallback(const sensor_msgs::msg::Image::ConstSharedPtr record_msg){
//   auto record_img = cv_bridge::toCvShare(record_msg, "rgb8")->image;
//   vw.write(record_img);
// }
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg, const int aim_mode_)
{
  ...
  if (debug_) {
    rune_debug_pub_.publish(
      cv_bridge::CvImage(img_msg->header, "mono8", rune_detector_->rune_debug).toImageMsg());
  }
  return rune;
}
